...
A basic API will be created, served at http(s)://yourdomian.whatever/api. Edit the "api.py" file to add calls. The default API contains a simple "build" call, accesible via a GET request. You can use this GET request as an endpoint for automation tools such as [Zapier](https://zapier.com/) - just set up your trigger (maybe a file being added or changed in a Google Drive folder) in your tool of choice and point it at https://yourdomian.whatever/api/build?action=run&password=yourPassword
